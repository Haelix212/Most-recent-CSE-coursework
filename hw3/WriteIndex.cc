...
int WriteIndex(MemIndex* mi, DocTable* dt, const char* file_name) {
  ...
  int he_bytes = WriteHeader(f, dt_bytes, mi_bytes);
  if (he_bytes == kFailedWrite) {
    fclose(f);
    unlink(file_name);
    return kFailedWrite;
  }
...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  CRC32 crc;
  int body_size = doctable_bytes + memidx_bytes;
  char buf[512];
  int res;
  ...
  for (int i = 0; i < body_size;) {
    res = fread(buf, sizeof(char), 512, f);
    if (res < 0) {
      return kFailedWrite;
    }
    for (int j = 0; j < res; j++) {
      crc.FoldByteIntoCRC(buf[j]);
    }
    i += res;
  }

  // // Reads characters one-by-one, inefficient
  // char c;
  // for (int i = 0; i < body_size; i++) {
  //   int res = fread(c, sizeof(char), 1, f);
  //   if (res != 1) {
  //     return kFailedWrite;
  //   }
  //   crc.FoldByteIntoCRC(*c);
  // }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  // printf(" &WriteDocidToDocenameFn : %p\n", &WriteDocidToDocnameFn);
  // printf(" &WriteWordToPostingsFn : %p\n", &WriteWordToPostingsFn);
  // printf("WriteHashTable 1, Fn : %p \n", fn);  // debugging
  int res, items;
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    LinkedList* list = (ht->buckets)[i];
    items = LinkedList_NumElements(list);
    // printf("  items : %d\n", items);
    res = WriteHTBucketRecord(f, record_pos, items, bucket_pos);
    // printf("  res : %d\n", res);
    record_pos += res;

    if (items != 0) {
      res = WriteHTBucket(f, bucket_pos, list, fn);
      if (res == kFailedWrite) {
        return kFailedWrite;
      }
      bucket_pos += res;
...
static int WriteHTBucketRecord(FILE* f, IndexFileOffset_t offset,
  ...
  BucketRecord bucket(num_elts, bucket_offset);
  bucket.ToDiskFormat();
  ...
  if (fwrite(&bucket, sizeof(BucketRecord), 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    // printf("step 7\n");
    HTKeyValue_t* element = NULL;
    LLIterator_Get(it, reinterpret_cast<void**>(&element));
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  char* file_name = reinterpret_cast<char*>(kv->value);
  ...
  fwrite(reinterpret_cast<char*>(kv->value), sizeof(char), file_name_bytes, f);
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  fwrite(&header, sizeof(DocIDElementHeader), 1, f);
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  fwrite(&header, sizeof(WordPostingsHeader), 1, f);

  // STEP 18.
  // Write the word itself, excluding the null terminator, in the right
  // place in the file.
  res = fseek(f, offset + sizeof(WordPostingsHeader), SEEK_SET);
  if (res != 0) {
    return kFailedWrite;
  }
  fwrite(word, sizeof(char), word_bytes, f);
