...
#define HASH_BUCKETS 16  // Max num buckets for Index and WordPostings tables
...
MemIndex* MemIndex_Allocate(void) {
  ...
  HashTable* index = HashTable_Allocate(HASH_BUCKETS);
...
void MemIndex_AddPostingList(MemIndex* index, char* word, DocID_t doc_id,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    Verify333(wp != NULL);
    wp->word = word;
    wp->postings = HashTable_Allocate(HASH_BUCKETS);
...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  while (HTIterator_IsValid(itr)) {
    ...
    result->rank = (uint32_t) LinkedList_NumElements((LinkedList*) kv.value);
  ...
  for (i = 1; i < query_len; i++) {
    ...
    if (HashTable_Find(index, key, &kv) == false) {
    ...
    for (j = 0; j < num_docs; j++) {
      ...
      if (HashTable_Find(wp->postings, result->doc_id, &kv) == false) {
        LLIterator_Remove(ll_it, &free);
      } else {
        result->rank += (int) LinkedList_NumElements(kv.value);
