...
static void HandleDir(char* dir_path, DIR* d, DocTable** doc_table,
  ...
  for (i = 0; dirent != NULL;) {
    ...
    if (strncmp(dirent->d_name, "..", strlen("..") + 1) == 0 ||
        strncmp(dirent->d_name, ".", strlen(".") + 1) == 0) {
      dirent = readdir(d);
      continue;
    }
    ...
    dirent = readdir(d);  // move to the next directory entry
...
static void HandleFile(char* file_path, DocTable** doc_table,
  ...
  file_contents = ReadFileToString(file_path, &file_len);
