...
DocID_t DocTable_Add(DocTable* table, char* doc_name) {
  ...
  res = FNVHash64((unsigned char*) doc_name, strlen(doc_name) + 1);
  if ((HashTable_Find(table->name_to_id, (HTKey_t) res, &old_kv) == true)) {
...
DocID_t DocTable_GetDocID(DocTable* table, char* doc_name) {
  ...
  key = FNVHash64((unsigned char*) doc_name, strlen(doc_name) + 1);
...
char* DocTable_GetDocName(DocTable* table, DocID_t doc_id) {
  ...
  if ((HashTable_Find(table->id_to_name, doc_id, &kv) == true)) {
